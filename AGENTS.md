...
- **Tests:** `npm test`（Vitest）；`npm run test:coverage` 额外写出 `coverage/lcov.info`（供日后 Sonar CI 扫描；见 `sonar-project.properties`）。
...
- Env vars (all optional for basic dev) are in `.env.example`; use a local `.env.local`. `PUBLIC_OUT_OF_SITE_LINK_HMAC_KEY` is recommended if you need to test outbound-link (`/out-of-site/`) handling, otherwise external-link clicks hit a 503 recovery page. Architecture notes: [`docs/out-of-site-interstitial.md`](docs/out-of-site-interstitial.md), [`docs/site-chrome-clients.md`](docs/site-chrome-clients.md).
- Site identity (title, url, Giscus, telemetry switches, source base URL) lives in [`umamichi.config.mjs`](umamichi.config.mjs); schema: [`docs/umamichi-config.md`](docs/umamichi-config.md). Deploy routes stay in `wrangler.jsonc`.
- Vitest uses a plain `vitest.config.ts` (not Astro `getViteConfig`) so `@vitest/coverage-v8` can load; unit tests under `src/lib` are framework-free.
