...
- **评论区（Giscus）**：在 `ArticleComments` 根节点上设置 **`data-out-of-site-ugc="giscus"`**。落在此容器内的外链（见下文化限制）使用 **`kind=giscus`**，并附带查询参数 **`hash`**：其为**对称密钥**下的 **HMAC-SHA256**（普通对称 MAC，与 `PUBLIC_OUTBOUND_LINK_HMAC_KEY` 共享同一秘密）。Giscus **不**使用非对称的 Ed25519 `sig`。
...
| `hash` | `kind=ssr` 与 `kind=giscus` 均必填 | **对称** MAC：对规范报文做 **HMAC-SHA256** 后的 Base64URL；与 `PUBLIC_OUTBOUND_LINK_HMAC_KEY` 共享秘密（收发同一密钥，非公私钥对）。 |
...
| `PUBLIC_OUTBOUND_LINK_HMAC_KEY` | 前端 | **`hash` 的对称密钥**（任意字符串）；`kind=ssr` 与 `kind=giscus` 共用。须与构建/运行环境一致。未设置则无法完成 MAC 校验，将跳转 **`/503/`** 恢复页。 |
